...
import 'package:admin_college/admin/upload_pdf.dart';
import 'package:flutter/material.dart';
import 'package:flutter/cupertino.dart';

class SelectUploading extends StatefulWidget {
  const SelectUploading({Key? key}) : super(key: key);

  @override
  _SelectUploadingState createState() => _SelectUploadingState();
}

class _SelectUploadingState extends State<SelectUploading> {
  String? selectedSubject; // Initially, no subject is selected


  List<CourseNCode> subjects = [
  ...
  void initState() {
    super.initState();
    // Sort the list alphabetically based on courseName
    subjects.sort((a, b) => a.courseName.compareTo(b.courseName));
  }
        ...
        child: Column(
          ...
          children: [
              ...
              DropdownButton<String>(
                ...
                items: subjects.map((CourseNCode subject) {
                  return DropdownMenuItem<String>(
                    value: subject.courseName,
                    child: Container(
                        width: 200,
                        child: Text(subject.courseName)),
                  );
                }).toList(),
              ),
            SizedBox(height: 20),
            ElevatedButton(
              onPressed: () {
                if (selectedSubject != null) {
                  CourseNCode selectedCourse = subjects.firstWhere((course) => course.courseName == selectedSubject);
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) {
                      return UploadPDFScreen(selectedCourse);
                    }),
                  );
                }
              },
